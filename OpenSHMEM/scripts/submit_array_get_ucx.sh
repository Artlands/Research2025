...
#SBATCH --output=results/get_mr_ucx_%a.out     # Standard output log (%j expands to job ID)
...
# Profile with UCX
UCX_PROFILE_MODE=accum UCX_PROFILE_FILE=./results/ucx_$MESSAGE_SIZE.prof \
  oshrun -np 2 --map-by ppr:1:node ./oshm_get_mr.exe $MESSAGE_SIZE

ucx_read_profile ./results/ucx_$MESSAGE_SIZE.prof > ./results/ucx_accum_$MESSAGE_SIZE.txt

rm -f ./results/ucx_$MESSAGE_SIZE.prof